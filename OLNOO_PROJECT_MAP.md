...
Fixed entirely at upload time, in `POST /api/social/upload-instagram-image` (the only place a file becomes `instagram_image_url`), not in the publish flow: after the existing magic-byte/size checks, the buffer goes through `normalizeInstagramImageBuffer` (`lib/social-instagram-image-normalize.ts`) before ever being written to disk. An already-valid image's bytes are returned completely unchanged — no re-encode, no quality loss, no behavior change for the common case (`isInstagramFeedRatioAllowed`). An out-of-range image is centered-cropped (never stretched — no distortion) down to exactly **4:5**, the single preferred normalized target for both directions: a too-tall portrait crops height, a too-wide landscape crops width, both computed by the pure, unit-tested `computeInstagramFeedCrop(width, height)`. Re-encoded via `jimp`'s JPEG encoder (`quality: 92`) and re-checked against the same 8MB cap the original upload already enforced. Because normalization happens before the file is saved, the same served URL (`GET /api/social/instagram-image/[project]/[filename]`) the post's preview `<img>` and the publish flow both use is always the final, already-normalized file — nothing downstream (preview, `social_posts.instagram_image_url`, `lib/social-instagram.ts`, `social_publications`) needed to change.
