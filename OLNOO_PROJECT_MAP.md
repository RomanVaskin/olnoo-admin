...
Post fields (create/edit UI): Topic, Content (shared body), Channels (`telegram`/`instagram`/`threads`/`vk`), Publish date, Status, and an optional per-channel text override per selected channel. `category` is a legacy column still present in the table — no longer shown in create/edit, the Posts list, or search; existing rows keep whatever value they already had, and new rows never get one written. When `instagram` is a selected channel, an additional "Instagram image URL" field (`social_posts.instagram_image_url`) appears — always visible while Instagram is selected, independent of the "Different text per channel" toggle, since it's mandatory content for Instagram publishing (see Instagram autoposting below), not a per-channel text override.
