...
Before the file is written to disk, it's passed through `lib/social-instagram-image-normalize.ts` (`normalizeInstagramImageBuffer`) to guarantee an Instagram Feed-valid aspect ratio — see "Instagram image aspect ratio normalization" below. That step doesn't add a new dependency on storage or a media service; it's a synchronous in-process transform using `jimp` (a pure-JS image library — no native binary, so nothing new for the KZ deploy's `npm ci` to compile), same request, same route, same disk.

### Instagram image aspect ratio normalization

Root cause this fixes: `lib/social-instagram.ts`'s `publishSocialPublicationToInstagram` (untouched by this) was rejecting real posts with Instagram's own `"The aspect ratio is not supported."` error — the upload step accepted any JPEG shape, but Instagram's Content Publishing API for Feed images only accepts an aspect ratio between **4:5 (portrait) and 1.91:1 (landscape) inclusive** (confirmed against Meta's current `developers.facebook.com/docs/instagram-platform/content-publishing/` docs — width outside 320–1440px is auto-scaled by Instagram's own servers, so only ratio needed handling here).

Fixed entirely at upload time, in `POST /api/social/upload-instagram-image` (the only place a file becomes `instagram_image_url`), not in the publish flow: after the existing magic-byte/size checks, the buffer goes through `normalizeInstagramImageBuffer` (`lib/social-instagram-image-normalize.ts`) before ever being written to disk. An already-valid image's bytes are returned completely unchanged — no re-encode, no quality loss, no behavior change for the common case (`isInstagramFeedRatioAllowed`). An out-of-range image is centered-cropped (never stretched — no distortion) to the nearer edge of the accepted range: a too-tall portrait (ratio < 4:5) crops height down to exactly **4:5**; a too-wide landscape (ratio > 1.91:1) crops width down to exactly **1.91:1** — cropping to the nearer edge rather than always 4:5 keeps as much of the original frame as the API allows. Both computed by the pure, unit-tested `computeInstagramFeedCrop(width, height)`. Re-encoded via `jimp`'s JPEG encoder (`quality: 92`) and re-checked against the same 8MB cap the original upload already enforced. Because normalization happens before the file is saved, the same served URL (`GET /api/social/instagram-image/[project]/[filename]`) the post's preview `<img>` and the publish flow both use is always the final, already-normalized file — nothing downstream (preview, `social_posts.instagram_image_url`, `lib/social-instagram.ts`, `social_publications`) needed to change.

`jimp` (pure JS, no native/binary dependency — confirmed via its own `package.json`, unlike e.g. `sharp`) is used only for this: decode buffer → read `width`/`height` → optionally `crop({x,y,w,h})` → `getBuffer('image/jpeg')`. Deliberately not `sharp` or another native-binding library, to avoid adding a platform-specific compiled dependency to the KZ deploy's plain `npm ci && npm run build`.
